...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    //获取字符串中文字的拼音首字母并与字符串共同存放
    NSMutableArray *chineseStringsArray=[NSMutableArray array];
	NSMutableArray * teshuArray = [NSMutableArray array];
    for(int i=0;i<[stringArr count];i++){
        BMChineseSort *chineseString = [[BMChineseSort alloc]init];
        //获取对象对应字段的字符串
		...
		if ([chineseString.pinYin isEqualToString:AbnormalLetter]) {
			[teshuArray addObject:chineseString];
		}else{
			  [chineseStringsArray addObject:chineseString];
		}
    }
    ...
    NSArray *sortDescriptors = [NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"pinYin" ascending:YES]];
    [chineseStringsArray sortUsingDescriptors:sortDescriptors];
	if (teshuArray.count > 0) {
		[chineseStringsArray addObjectsFromArray:teshuArray];
	}
    return chineseStringsArray;
}
